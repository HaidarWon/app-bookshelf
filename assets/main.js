...
const formAddingBook = document.getElementById("tambahDataBuku");
const formSearchingBook = document.getElementById("cariDataBuku");

function CheckForStorage() {
    return typeof Storage !== "undefined";
}

formAddingBook.addEventListener("submit", function () {
    const title = document.getElementById("judul_buku").value;
    const author = document.getElementById("penulis_buku").value;
    const year = parseInt(document.getElementById("tahun_buku").value);
    const isComplete = document.getElementById("inputBookIsComplete").checked;
    const idTemp = document.getElementById("judul_buku").name;
    if (idTemp !== "") {
    const bookData = GetBookList();
    for (let form = 0; form < bookData.length; form++) {
        if (bookData[form].id == idTemp) {
            bookData[form].title = title;
            bookData[form].author = author;
            bookData[form].year = year;
            bookData[form].isComplete = isComplete;
        }
    }
...
function CreateGreenButton(book, eventListener) {
    const isSelesai = book.isComplete ? "Belum selesai" : "Sudah selesai";

    const greenButton = document.createElement("button");
    greenButton.classList.add("green");
    greenButton.innerText = isSelesai + " dibaca";
    greenButton.addEventListener("click", function (event) {
    eventListener(event);
    alert("DATA BERHASIL BERUBAH !");
    });
    return greenButton;
}
...
function isCompleteBookHandler(itemElement) {
    ...
    for (let form = 0; form < bookData.length; form++) {
    if (bookData[form].title === title && bookData[form].id == titleNameAttribut) {
        bookData[form].isComplete = !bookData[form].isComplete;
        break;
    }
    }
    localStorage.setItem(storageKey, JSON.stringify(bookData));
...
function SearchBookList(title) {
    ...
    for (let form = 0; form < bookData.length; form++) {
    const tempTitle = bookData[form].title.toLowerCase();
    const tempTitleTarget = title.toLowerCase();
        if (bookData[form].title.includes(title) || tempTitle.includes(tempTitleTarget)) {
            bookList.push(bookData[form]);
        }
    }
    return bookList;
}
...
function UpdateAnItem(itemElement) {
    ...
    document.getElementById("judul_buku").value = title;
    document.getElementById("judul_buku").name = id;
    document.getElementById("penulis_buku").value = author;
    document.getElementById("tahun_buku").value = year;
    document.getElementById("inputBookIsComplete").checked = isComplete;

    for (let form = 0; form < bookData.length; form++) {
    if (bookData[form].id == id) {
        bookData[form].id = id;
        bookData[form].title = title;
        bookData[form].author = author;
        bookData[form].year = year;
...
cariDataBuku.addEventListener("submit", function (event) {
    ...
    const title = document.getElementById("cariJudulBuku").value;
    if (title === null) {
        RenderBookList(bookData);
    return;
    }
...
function ResetAllForm() {
    ...
    document.getElementById("penulis_buku").value = "";
